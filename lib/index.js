...
function fetch(callback) {
  ...
  request(NOAA, function (error, response, body) {
    if (!error && response.statusCode == 200) {
      ...
      if (!link) return callback('no hurricanes found');
...
/**
 * Find the nearest coast to the center of the cyclone
 * @param {array} coordinates [lng, lat]
 * @return {object} breakpoint { name, lng, lat }
 */
function nearestCoast(coordinates) {
  var buffer = fs.readFileSync(__dirname + '/../data/Breakpoints_2016.geojson');
  var breakpoints = JSON.parse(buffer.toString());
  ...
  return nearest(center, breakpoints);
}
...
module.exports.nearestCoast = nearestCoast;
